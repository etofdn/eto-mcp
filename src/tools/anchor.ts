...
async function sh(cmd: string, cwd?: string): Promise<{ stdout: string; stderr: string }> {
  return run(cmd, { cwd: cwd || WORK_DIR, timeout: 120000 });
}
...
export function registerAnchorTools(server: McpServer): void {
  server.tool(
    ...
    async ({ name }) => {
      try {
        ...
        const { stdout, stderr } = await sh(`anchor init ${name} 2>&1`, WORK_DIR);
  ...
  server.tool(
    ...
    async ({ source, program_name }) => {
      try {
        ...
        const projectDir = `${WORK_DIR}/${name}`;
        ...
        if (!existsSync(projectDir)) {
          await sh(`anchor init ${name} 2>&1`, WORK_DIR);
        }
        ...
        // Build
        const { stdout, stderr } = await sh("anchor build 2>&1", projectDir);
  ...
  server.tool(
    ...
    async ({ program_name, test_source }) => {
      try {
        const projectDir = `${WORK_DIR}/${program_name}`;
        if (!existsSync(projectDir)) {
        ...
        const { stdout, stderr } = await sh("anchor test --skip-local-validator 2>&1", projectDir);
